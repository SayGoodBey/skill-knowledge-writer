...
- **问题与解决**: 遇到了什么问题、如何解决
- **经验教训**: 踩了什么坑、有什么最佳实践
- **流程图 / 架构图**: 对话中出现的图片流程图、时序图、架构图、状态机等，**不要丢弃**，统一转写为 Mermaid 代码记录（详见 3.4）
...
#### 3.4 流程图转 Mermaid（关键，不可丢弃！）

对话中如果出现**图片形式的流程图、时序图、架构图、状态机、ER 图**等，**不能直接丢弃**。应理解图片内容后，转写为 [Mermaid](https://mermaid.js.org/) 代码块写入对应 topic / daily 文件。VitePress 原生支持 Mermaid 渲染，这样比贴图更易读、可检索、可维护。

转写原则：
- **看懂再转**：先理解图的节点、连线、方向、分支条件，保证语义等价，不要凭空编造
- **选对图类型**：
  - 流程 / 步骤 → `flowchart TD`（或 `LR`）
  - 时序 / 交互 → `sequenceDiagram`
  - 状态流转 → `stateDiagram-v2`
  - 数据模型 / 表关系 → `erDiagram`
  - 时间计划 → `gantt`
- **保留中文标签**：节点文字用原图中文，必要时补一句文字说明图的用途
- **无法识别**时不要硬编，记录文字描述并备注"原图未能完整还原"

示例：

```markdown
**流程图**（源自对话图片，已转写为 Mermaid）:
...
7. **流程图必转 Mermaid**: ⚠️ 对话中的图片流程图/架构图/时序图等不可丢弃，须转写为 Mermaid 代码块（见 3.4），保证语义等价、保留中文标签
